...
public class tFlocking : MonoBehaviour 
{
    ...
    private GameObject[] _flock_last_frame;
	private GameObject _selection			= null;

	private Color _selection_color	 		= new Color(0.0f, 1.0f, 0.0f, 1.0f);
	private Color _default_color			= new Color(1.0f, 1.0f, 1.0f, 1.0f);
	...
	void Start () 
	{
        _flock = new GameObject[_COUNT];
        _flock_last_frame = new GameObject[_COUNT];

        //initialize the flock array of gameobjects 
        for (int i = 0; i < _flock.Length; i++)
		{
            //random scale value
            randomRange = Random.Range(RandomScale[0], RandomScale[1]);

            //_flock[i] 							= GameObject.CreatePrimitive(PrimitiveType.Cube);
            _flock[i]                           = Instantiate(flockObject);
			_flock[i].transform.localScale		= Vector3.one * randomRange;
            ...
            _flock[i].GetComponentInChildren<Renderer>().material.color = _default_color;
		}
